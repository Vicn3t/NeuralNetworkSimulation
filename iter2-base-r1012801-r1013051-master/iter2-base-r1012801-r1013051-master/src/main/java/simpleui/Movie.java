...
public class Movie extends JComponent {
    ...
    private void renderSurvivalAndCreatures() {
    	...
    	bfiRenderer.clearPixels(bufferedImage);
    	
    	bfiRenderer.renderSurvivalZone(bufferedImage, sim.getWorld(), sim.getNaturalSelection());

    	for (Creature creature : sim.getWorld().getPopulation()) {
        bfiRenderer.renderCreature(bufferedImage, creature.getPosition(), creature.getBehavior().getColor()); 
    	}
